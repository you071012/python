...
class RefershTmp1():
    def __init__(self):
        ...
        self.tag = str(argv[2])
        self.last_acct_date = (dt.strptime(self.acct_date, "%Y%m%d").date() + datetime.timedelta(days=-1)).strftime("%Y%m%d")
    ...
    def refersh(self):
        ...
        qry_sql = "select acct_no from acct_info where acct_date_tmp2 in ('%s','%s') and acct_date_tmp1 < acct_date_tmp2" % (self.last_acct_date,self.acct_date)
